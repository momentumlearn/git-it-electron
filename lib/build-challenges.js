...
function buildSidebar(filename, lang) {
  ...
  const challenges = Object.keys(require('../empty-data.json')).map(function (
    title
  ) {
...
function getPartial(filename, lang) {
  try {
    const pos = path.join(
      locale.getLocaleResourcesPath(lang),
    ...
    return path.join(
      locale.getLocaleResourcesPath(locale.getFallbackLocale()),
      'partials/' + filename + '.html'
    )
  }
}
