...
const resetAllChallenges = () => {
  console.log('resetting all challenges')
  const data = getDataFromFile()
  for (let key in data.contents) {
    data.contents[key].completed = false
  }
  writeToFile(data)
}

const updateCurrentDirectory = (path) => {
  const data = getSavedDir()
  data.contents.savedDir = path
  writeToFile(data)
}

const updateCompletedChallenges = (challenge) => {
  const data = getDataFromFile()
  data.completedChallenges = [...data.completedChallenges, challenge]
  writeData(data)
}
...
module.exports.resetAllChallenges = resetAllChallenges
